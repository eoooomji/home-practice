...
### 12-2. Node
    ...
    - apply
        ...
        : 일반적인 함수와 마찬가지로 매개변수로 직접받지만 apply는 매개변수를 '배열'로 받는다.
    - bind : call과 사용방법은 같으나 실행을 해야한다.

    - checkbox
        ◦ checked, selected : 논리값으로 출력이 된다.(true, false)

    - HTML과 JS의 연결되는 태그 속성 값
        (1) usemap : 요소와 연결할 이미지 맵의 이름(프래그먼트)
        (2) map : area 요소와 함께 이미지 맵(클릭 가능한 링크 영역)을 정의할 때 사용
        (3) area : 미지의 핫스팟 영역을 정의하고 하이퍼링크 (en-US)를 추가. map요소에서 사용 가능
            - area 특성
                ◦ coords : 핫스팟 영역을 지정하는 일련의 좌표
                ◦ alt : 이미지를 출력하지 않는 브라우저에서 대신 표시할 대안 텍스트
                ◦ shape : 관련된 핫 스폿의 모양(rect:직사각형, circle:원형, poly : 다각형 )

---

## 12. Async Function 비동기 함수

: 먼저 실행된 코드의 결과와 상관없이 다음 동작을 병렬적으로 처리

: 대부분의 프로그래밍은 동기적 처리가 기본이나, 긴 시간이 소요되는 기능은 비동기적으로 작동할 수 있도록 처리해주어야 함

: setTimeout, addEventListener, XMLHttpRequest

: 자바스크립트에서 비동기를 실행하고 그 처리가 끝난 후 다음 처리를 실행하기 위한 용도

: 비동기 실행 순서 제어 : callback, promise, Async/Await

---

    - setTimeout(실행함수, 시간(1000=1초))
        : milliseconds 만큼 대기한 후, 함수를 실행.
        : callback 함수를 많이 사용할 경우 가독성이 떨어진다. promise 로 대체하여 사용
        : 일정한 시간이 지난 후 한 번만 실행함
    - setInterval(실행함수, 시간) : 일정 시간 간격을 두고 반복 실행함
    - addEventListener : 지정한 유형의 이벤트를 대상이 수신할 때마다 호출할 함수
    - XMLHttpRequset
        : AJAX(Asynchronous JavaScript And XML) 프로그래밍에 많이 사용된다.
        : 서버와 상호작용할 때 사용
        ...
        ◦ send
            : 요청을 서버로 보낸다.
            : 요청이 비동기인 경우(기본값) 이 메서드는 요청이 전송되고 이벤트를 사용하여 결과가 전달되는 즉시 반환된다.
